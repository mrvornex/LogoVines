...
import { CATEGORIES, slugToLabel, slugToDbValue } from "@/lib/categories";
...
export default async function CategoryPage({ params }: Props) {
  ...
  // "all" → no filter, else exact DB value match
  const query =
    slug === "all"
      ? {}
      : { category: slugToDbValue(slug) };
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen">
          ...
          <div className="flex items-end justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-2">
                {logos.length} logo{logos.length !== 1 ? "s" : ""} found
              </p>
            </div>
              ...
              <Link
                ...
                className={`px-4 py-1.5 rounded-full text-[10px] uppercase tracking-widest border transition ${
                  slug === "all"
                    ...
                    : "border-white/10 text-gray-500 hover:border-[#d4a373] hover:text-white"
                }`}
              >
                All
              </Link>
      ...
      <div className="text-center py-12">
        <Link href="/#categories" className="text-gray-600 hover:text-[#d4a373] text-xs uppercase tracking-widest transition">
          ← Back to Categories
        </Link>
      </div>
    </div>
