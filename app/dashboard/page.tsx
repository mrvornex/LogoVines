...
function DashboardContent() {
  ...
  const saveAccount = async () => {
    ...
    const data = await res.json();
    if (data.success) { setUser((u: any) => ({...u, ...account})); flash("success", "Account saved!"); }
      ...
      <div className="pt-24 max-w-5xl mx-auto px-6 pb-20">
        ...
        <div className="flex items-center border-b border-gray-200 mb-6">
          {(["uploads","profile","account"] as Tab[]).map((k) => (
            ...
            >
              {k === "uploads" ? "My Uploads" : k === "profile" ? "Profile Settings" : "Edit Account"}
            </button>
          ))}
        ...
        {tab === "uploads" && (
          <div>
            <div className="flex gap-3 mb-5">
              ...
              <Link href="/upload" className="ml-auto self-center bg-[#1A4450] text-white px-4 py-2.5 text-xs font-bold uppercase tracking-widest hover:bg-[#1A4450]/80 transition rounded-lg">
                + Upload New
              </Link>
            </div>

            <div className="border border-gray-200 rounded-xl overflow-hidden">
              <div className="grid px-5 py-3 border-b border-gray-100 bg-gray-50 text-gray-400 text-[10px] uppercase tracking-widest"
                style={{ gridTemplateColumns: "52px 40px 1fr 110px 100px 90px" }}>
                <span>Thumb</span><span>ID</span><span>Logo Name</span><span>Date</span><span>Category</span><span>Status</span>
              </div>
              ...
              {logos.length === 0 ? (
                ...
                <div key={logo._id}
                  className="grid px-5 py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition items-center"
                  style={{ gridTemplateColumns: "52px 40px 1fr 110px 100px 90px" }}
                >
                  ...
                  <span className="text-gray-400 text-[10px] font-mono">#{i+1}</span>
                  ...
                  <span className="text-gray-400 text-xs">{new Date(logo.createdAt).toLocaleDateString("en-PK", {day:"numeric",month:"short",year:"numeric"})}</span>
                  <span className="text-gray-400 text-xs capitalize">{logo.type || "brand"}</span>
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input type="url" placeholder={`https://${f.key}.com/yourprofile`}
                  value={(profile as any)[f.key]} onChange={(e) => setProfile({...profile, [f.key]:e.target.value})}
          ...
          <div className="max-w-lg space-y-6">
            ...
            <div className="border border-gray-200 rounded-xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-[#1A4450] font-bold text-sm">Account Settings</h3>
                <span className="text-gray-400 text-xs">
                  Joined: {user?.createdAt ? new Date(user.createdAt).toLocaleDateString("en-PK", {day:"numeric",month:"long",year:"numeric"}) : "—"}
                </span>
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={account.name} onChange={(e) => setAccount({...account, name:e.target.value})} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Email</label>
                <input type="email" value={account.email} onChange={(e) => setAccount({...account, email:e.target.value})} className={inputClass} />
              </div>
            ...
            {/* Change Password */}
            <div className="border border-gray-200 rounded-xl p-5 space-y-4">
              <h3 className="text-[#1A4450] font-bold text-sm">Change Password</h3>
              {[
                ...
                {label:"Confirm Password", key:"confirm", show:showPass.cf,toggle:() => setShowPass(s => ({...s,cf:!s.cf}))},
              ].map((f) => (
                <div key={f.key}>
                  <label className={labelClass}>{f.label}</label>
                  <div className="relative">
                    ...
                    <button type="button" onClick={f.toggle} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#1A4450]">
                      {f.show
