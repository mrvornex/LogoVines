...
export default function UserUploadForm() {
  ...
  const submitSingle = async () => {
    ...
    if (data.success) {
      ...
      setTimeout(() => router.push("/dashboard?tab=uploads"), 2000);
  ...
  const submitFolder = async () => {
    if (folderImages.length === 0) return setError("Please select a folder with images");
    if (!folderName.trim())        return setError("Collection name is required");
    ...
    if (data.success) {
      setSuccess(`${data.count} logos submitted for review! You'll be notified once approved.`);
      setTimeout(() => router.push("/dashboard?tab=uploads"), 2000);
    } else if (res.status === 401) {
      router.push("/user-login");
    } else {
      ...
      <div className="min-h-[80vh] py-12 px-4">
        <div className="max-w-4xl mx-auto">
            ...
            {(["brand", "template"] as const).map((t) => (
              <button key={t} onClick={() => { setUploadType(t); setSelectedCat(""); }}
                className={`flex-1 py-3 text-xs font-semibold uppercase tracking-widest transition ${
                  uploadType === t ? "bg-[#d4a373] text-black" : "bg-[#111] text-gray-400 hover:text-white"
                }`}
              >{t}</button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {cats.map((cat: any) => (
              <button key={cat.slug}
                onClick={() => { setSelectedCat(cat.dbValue); reset(); setStep("upload"); }}
    ...
    <div className="py-10 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          <button onClick={() => { setSelectedCat(""); setStep("choose"); reset(); }}
        ...
        <div className="flex border border-white/10 rounded-xl overflow-hidden mb-8">
          {(["file", "folder"] as const).map((t) => (
            <button key={t} type="button"
              ...
              style={tab === t ? { background: (currentCat as any)?.color || "#d4a373" } : {}}
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-[#111] border border-white/5 rounded-xl p-4 text-xs text-gray-500 leading-relaxed">
            📋 <strong className="text-gray-400">Guidelines:</strong> Only original work. No copyrighted logos. Min 500px resolution. Admin reviews within 24-48 hours.
          </div>

          <button type="submit" disabled={loading}
            className="w-full py-4 text-black font-bold uppercase tracking-widest text-sm rounded-xl transition disabled:opacity-50 flex items-center justify-center gap-2"
            style={{ background: loading ? "#888" : ((currentCat as any)?.color || "#d4a373") }}
          >
            {loading ? (
              <><svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeDasharray="60" strokeDashoffset="20"/></svg>Submitting...</>
            ) : tab === "file" ? "Submit for Review" : `Submit ${folderImages.length > 0 ? folderImages.length + " Logos" : "Folder"} for Review`}
          </button>
        </form>
      </div>
    </div>
